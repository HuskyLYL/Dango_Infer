#ifndef DANGO_ROPE_KERNEL_CU_CUH
#define DANGO_ROPE_KERNEL_CU_CUH
...
namespace base_kernel_cu 
{
    void rope_kernel_cu(int32_t dim, int32_t kv_dim, int32_t head_size, const tensor::Tensor& input_q,
        ...
        const tensor::Tensor& sin_cache, const tensor::Tensor& cos_cache, cudaStream_t stream=nullptr);

    void sin_cos_cache_calc_cu(int head_size, int max_seq_len, const tensor::Tensor& sin_cache,
        const tensor::Tensor& cos_cache, cudaStream_t stream =nullptr);
...
#endif  // DANGO_ROPE_KERNEL_CU_CUH
