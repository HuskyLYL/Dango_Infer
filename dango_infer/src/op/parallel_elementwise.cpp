...
namespace op
{
    ParallelVecAddLayer::ParallelVecAddLayer()
        : VecAddLayer()
    {
        this->set_layer_name("ParallelAdd");
    }

    base::Status ParallelVecAddLayer::check() const
    {
        CHECK_GT(nccl::G_MPI_SIZE, 0);
        return VecAddLayer::check();
    }

    base::Status ParallelVecAddLayer::forward(cudaStream_t stream)
    {
        return VecAddLayer::forward(stream);
    }

    base::Status ParallelVecAddLayer::forward(const tensor::Tensor& input1, const tensor::Tensor& input2,
        const tensor::Tensor& output1, cudaStream_t stream)
    {
        CHECK_GT(nccl::G_MPI_SIZE, 0);

        const size_t total_elems = input1.size();
        CHECK_EQ(total_elems, input2.size());
        CHECK_EQ(total_elems, output1.size());
        CHECK_EQ(total_elems % static_cast<size_t>(nccl::G_MPI_SIZE), 0u)
            << "ParallelVecAdd input size must be divisible by world size";
        ...
        auto status = VecAddLayer::forward(stream);
        if (!status)
            return status;

        nccl::TensorAllGather(this->get_output(0), stream);
